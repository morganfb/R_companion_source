...
\begin{alltt}
\hlkwd{par}\hlstd{(}\hlkwc{mfrow} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,} \hlnum{2}\hlstd{))}
\hlcom{## CHANGE THIS TO DATA FROM FIGURE 6.3}
\hlkwd{hist}\hlstd{(}\hlkwd{resid}\hlstd{(}\hlkwd{lm}\hlstd{(frog.altitude.data}\hlopt{$}\hlstd{clutch.volume} \hlopt{~} \hlstd{frog.altitude.data}\hlopt{$}\hlstd{body.size)),}
    \hlkwc{main} \hlstd{=} \hlstr{""}\hlstd{)}
\hlkwd{qqnorm}\hlstd{(}\hlkwd{resid}\hlstd{(}\hlkwd{lm}\hlstd{(frog.altitude.data}\hlopt{$}\hlstd{clutch.volume} \hlopt{~} \hlstd{frog.altitude.data}\hlopt{$}\hlstd{body.size)))}
\hlkwd{qqline}\hlstd{(}\hlkwd{resid}\hlstd{(}\hlkwd{lm}\hlstd{(frog.altitude.data}\hlopt{$}\hlstd{clutch.volume} \hlopt{~} \hlstd{frog.altitude.data}\hlopt{$}\hlstd{body.size)))}
\end{alltt}
\end{kframe}
