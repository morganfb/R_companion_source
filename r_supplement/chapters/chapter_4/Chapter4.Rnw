...
## Figure 4.4 
par(mfrow=c(2,2))
hist(samp$height, main = "Height")
hist(samp$weight, main = "Weight")
hist(samp$physically.active.7d, main = "Physical Activity in Past Week")
hist(samp$strength.training.7d, main = "Lifting Weights in Past Week")
@

\section{Variability in Estimates}
The sample parameters can be calculated as well as the population parameters.  
<<>>=
mean(samp$physically.active.7d, na.rm = TRUE)
mean(yrbss$physically.active.7d, na.rm = TRUE) 

median(samp$physically.active.7d, na.rm = TRUE)
median(yrbss$physically.active.7d, na.rm = TRUE) 

sd(samp$physically.active.7d, na.rm = TRUE)
...
for(ii in 1:1000){
  ...
  means[ii] = mean(samp$physically.active.7d, na.rm = TRUE)
}
...
<<>>= 
## Example 4.3 
xbar = mean(samp$physically.active.7d, na.rm = TRUE)
std.error = sd(samp$physically.active.7d, na.rm = TRUE)/sqrt(length(samp$physically.active.7d))
...
# Secondly, remove the people with missing BMI
# missing = which(is.na(nhanes.sample$BMI))  #Find missing values
# nhanes.sample = nhanes.sample[-missing, ]       #Remove them from the sample
